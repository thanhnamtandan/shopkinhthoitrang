...
if (!empty($priceFilters)) {
    ...
    if (in_array('under500k', $priceFilters)) {
        $priceConditions[] = 'discount < 500000';
    }
    if (in_array('500kto1m', $priceFilters)) {
        $priceConditions[] = 'discount BETWEEN 500000 AND 1000000';
    }
    if (in_array('above1m', $priceFilters)) {
        $priceConditions[] = 'discount > 1000000';
    }
...
// Bỏ từ khoá WHERE vì điều kiện sẽ được nối trực tiếp
$discountCondition = 'price != discount';

$totalQuery = "SELECT COUNT(*) as total FROM product WHERE " . $discountCondition . $priceQueryPart;
...
$querySanPham = "SELECT * FROM product WHERE " . $discountCondition . $priceQueryPart . $orderBy . " LIMIT $offset, $productsPerPage";
                                                    ...
                                                    <div class="card-content-box">
                                                        ...
                                                        <div class="price">
				                                            <span class="woocommerce-Price-amount amount"
                                                                <bdi><del style=" color: #777a7b; ">' . number_format($row[3], 0, '.', ',') . '</del><span class="woocommerce-Price-currencySymbol"></span></bdi>
                                                                <bdi>' . number_format($row[4], 0, '.', ',') . '<span class="woocommerce-Price-currencySymbol"> VNĐ</span></bdi>
                                                            </span>
                                                        </div>
		                                            </div>
                                                </div>
